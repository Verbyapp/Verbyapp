...
const GOLD_TROPHY_IMG =
	"https://lichess1.org/assets/hashed/gold-cup-2.e1e2ac3f.png";
const SILVER_TROPHY_IMG =
	"https://lichess1.org/assets/hashed/silver-cup-2.d820d24e.png";

const CommunityStreak = () => {
	...
	useEffect(() => {
		const leaderboardRef = query(
			ref(database, "users"),
			orderByChild("stats/streak/best"),
		);
		...
		const unsubscribe = onValue(leaderboardRef, (snapshot) => {
			if (snapshot.exists()) {
				...
				snapshot.forEach((child) => {
					...
					const stats = data.stats?.streak;

					const bestStreak = stats?.best ?? stats?.bestStreak ?? 0;

					if (stats && (stats.games > 0 || bestStreak > 0)) {
						users.push({
							uid: child.key,
	...
	const getRankTier = (position) => {
		if (position === 1) return { name: "Immortal", color: "#FFD700" };
		if (position === 2) return { name: "Legend", color: "#C0C0C0" };
		if (position === 3) return { name: "Elite", color: "#CD7F32" };
			...
			<div className="max-w-4xl mx-auto px-4 py-6 md:px-6 md:py-10">
				<div className="flex items-center gap-3 mb-2">
					<div className="w-8 h-8 md:w-10 md:h-10 rounded-lg bg-[#EB3514] flex items-center justify-center">
						<Flame size={16} className="md:size-5 text-white" />
					</div>
				...
				<p className="text-xs md:text-sm text-gray-500 mb-6 md:mb-8">
					Top players ranked by their best streak
				</p>
				...
				<div className="grid grid-cols-2 md:grid-cols-3 gap-2 md:gap-4 mb-6 md:mb-8">
					<div className="px-3 md:px-5 py-2 md:py-3 bg-[#F0EFEB] border border-[#DEDDDA] rounded-md flex items-center gap-2 md:gap-3">
						...
						<span className="text-[9px] md:text-xs text-gray-500 uppercase tracking-wide">
							Players
						</span>
					</div>
					...
					{players[0] && (
						<Link
							...
							/>
							<div>
								<div className="text-[9px] md:text-xs text-white/80 uppercase tracking-wide">
									Leader
								</div>
					...
					<div className="bg-[#F0EFEB] border border-[#DEDDDA] rounded-lg p-4 md:p-6">
						<div className="space-y-2">
							{[1, 2, 3, 4, 5].map((i) => (
								<div
									key={i}
									className="h-12 md:h-14 bg-white border border-[#DEDDDA] rounded animate-pulse"
								/>
							))}
						</div>
					</div>
					...
					<>
						<div className="hidden md:block bg-[#F0EFEB] border border-[#DEDDDA] rounded-lg overflow-hidden">
							<div className="grid grid-cols-12 gap-4 px-5 py-3 border-b border-[#DEDDDA] bg-white">
								...
								<div className="col-span-1 text-xs font-bold text-gray-500 uppercase tracking-wider text-right">
									W%
								</div>
							</div>
										...
										<div className="col-span-1 flex items-center justify-end">
											<span
												className={`text-xs font-medium ${
													player.games > 0 && player.wins / player.games >= 0.5
														? "text-green-600"
														: "text-gray-400"
												}`}>
												{player.games > 0
...
export default CommunityStreak;
